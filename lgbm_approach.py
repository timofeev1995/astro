...
def process_test(clfs, 
    ...
    meta_test = process_meta('data/test_set_metadata.csv.zip')
    # meta_test.set_index('object_id',inplace=True)

    remain_df = None
    for i_c, df in enumerate(pd.read_csv('data/test_set.csv.zip', chunksize=chunks, iterator=True)):
        # Check object_ids
...
def main(argc, argv):
    ...
    # agg features
    aggs = {
        'flux': ['min', 'max', 'mean', 'median', 'std','skew', 'size'],
        'flux_err': ['min', 'max', 'mean', 'median', 'std','skew'],
        'detected': ['mean'],
        'flux_ratio_sq':['sum', 'skew', 'mean', 'median', 'std'],
        'flux_by_flux_ratio_sq':['sum', 'skew', 'mean', 'median', 'std'],
    }
    ...
    print('Data reading')
    train = pd.read_csv('data/training_set.csv.zip')
    print('Featurizing started')
    full_train = featurize(train, meta_train, aggs, fcp)
    print('Featurizing is done')
    ...
    eval_func_lgbm = partial(lgbm_modeling_cross_validation, 
    ...
    eval_func_xgb = partial(xgb_modeling_cross_validation, 
                        full_train=full_train, 
                        y=y, 
                        classes=classes, 
                        class_weights=class_weights, 
    ...
    best_params.update({'n_estimators': 5000})
    
    # modeling from CV
    clfs_l, score_l = eval_func_lgbm(best_params)
    #clfs_x, score_x = eval_func_xgb(best_params)
    print(score_l)
    
    clfs = clfs_l
    
    filename = 'subm_{:.6f}_{}.csv'.format(score_l, 
